...
static int reg_int_cb(struct int_param_s *int_param) /* 中断回调函数（未实现） */
{
...
static uint8_t g_atk_ms6050_last_init_step = 0;
static uint8_t g_atk_ms6050_last_mpu_init_substep = 0;
static uint8_t g_atk_ms6050_last_mpu6500_rev = 0xFF;
static uint8_t g_atk_ms6050_last_fifo_step = 0;
static uint8_t g_atk_ms6050_last_fifo_reset_step = 0;

#if defined MPU6050
const struct gyro_reg_s reg = {
    0x75, 0x19, 0x1A, 0x0C, 0x6A, 0x23, 0x1B, 0x1C,
    0x00, 0x00, 0x1F, 0x20, 0x72, 0x74, 0x43, 0x3B,
    0x41, 0x38, 0x39, 0x3A, 0x00, 0x6B, 0x6C, 0x37,
    0x6F, 0x06, 0x24, 0x6D, 0x6E, 0x70
#ifdef AK89xx_SECONDARY
    ,.raw_compass   = 0x49,
    .yg_offs_tc     = 0x01,
    .s0_addr        = 0x25,
...
const struct gyro_reg_s reg = {
    0x75, 0x19, 0x1A, 0x0C, 0x6A, 0x23, 0x1B, 0x1C,
    0x1D, 0x1E, 0x1F, 0x20, 0x72, 0x74, 0x43, 0x3B,
    0x41, 0x38, 0x39, 0x3A, 0x69, 0x6B, 0x6C, 0x37,
    0x6F, 0x77, 0x24, 0x6D, 0x6E, 0x70
#ifdef AK89xx_SECONDARY
    ,.raw_compass   = 0x49,
    .s0_addr        = 0x25,
    .s0_reg         = 0x26,
...
int mpu_init(struct int_param_s *int_param)
{
    ...
    g_atk_ms6050_last_mpu_init_substep = 1;
    g_atk_ms6050_last_mpu6500_rev = 0xFF;
    ...
    g_atk_ms6050_last_mpu_init_substep = 2;
    ...
    /* The MPU6500 path reads internal memory before the common config block
     * below marks cached fields invalid. Prime sensors here so mpu_read_mem()
     * does not reject the access during early init.
     */
    st.chip_cfg.sensors = 0xFF;

#if defined MPU6050
    g_atk_ms6050_last_mpu_init_substep = 3;
...
#define MPU6500_MEM_REV_ADDR    (0x17)
    ...
    if (mpu_read_mem(MPU6500_MEM_REV_ADDR, 1, &rev))
        return -1;
    g_atk_ms6050_last_mpu6500_rev = rev;
    g_atk_ms6050_last_mpu_init_substep = 4;
    st.chip_cfg.accel_half = 0;
    ...
    g_atk_ms6050_last_mpu_init_substep = 5;
    if (i2c_write(st.hw->addr, st.reg->accel_cfg2, 1, data))
        return -1;
#endif

    g_atk_ms6050_last_mpu_init_substep = 6;
    ...
    g_atk_ms6050_last_mpu_init_substep = 7;
    if (mpu_set_gyro_fsr(2000))
        return -1;
    g_atk_ms6050_last_mpu_init_substep = 8;
    if (mpu_set_accel_fsr(2))
        return -1;
    g_atk_ms6050_last_mpu_init_substep = 9;
    if (mpu_set_lpf(42))
        return -1;
    g_atk_ms6050_last_mpu_init_substep = 10;
    if (mpu_set_sample_rate(50))
        return -1;
    g_atk_ms6050_last_mpu_init_substep = 11;
    if (mpu_configure_fifo(0))
        return -1;

    g_atk_ms6050_last_mpu_init_substep = 12;
    if (int_param)
    ...
    g_atk_ms6050_last_mpu_init_substep = 13;
    if (mpu_set_bypass(0))
        return -1;
#endif

    g_atk_ms6050_last_mpu_init_substep = 14;
    mpu_set_sensors(0);
    g_atk_ms6050_last_mpu_init_substep = 0;
    return 0;
}
...
int mpu_reset_fifo(void)
{
    ...
    g_atk_ms6050_last_fifo_reset_step = 1;

    if (!(st.chip_cfg.sensors))
        return -1;

    data = 0;
    g_atk_ms6050_last_fifo_reset_step = 2;
    if (i2c_write(st.hw->addr, st.reg->int_enable, 1, &data))
        return -1;
    g_atk_ms6050_last_fifo_reset_step = 3;
    if (i2c_write(st.hw->addr, st.reg->fifo_en, 1, &data))
        return -1;
    g_atk_ms6050_last_fifo_reset_step = 4;
    if (i2c_write(st.hw->addr, st.reg->user_ctrl, 1, &data))
        return -1;

    if (st.chip_cfg.dmp_on) {
        data = BIT_FIFO_RST | BIT_DMP_RST;
        g_atk_ms6050_last_fifo_reset_step = 5;
        if (i2c_write(st.hw->addr, st.reg->user_ctrl, 1, &data))
            return -1;
        delay_ms(50);
        ...
        if (st.chip_cfg.sensors & INV_XYZ_COMPASS)
            data |= BIT_AUX_IF_EN;
        g_atk_ms6050_last_fifo_reset_step = 6;
        if (i2c_write(st.hw->addr, st.reg->user_ctrl, 1, &data))
            return -1;
        if (st.chip_cfg.int_enable)
            ...
            data = 0;
        g_atk_ms6050_last_fifo_reset_step = 7;
        if (i2c_write(st.hw->addr, st.reg->int_enable, 1, &data))
            return -1;
        data = 0;
        g_atk_ms6050_last_fifo_reset_step = 8;
        if (i2c_write(st.hw->addr, st.reg->fifo_en, 1, &data))
            return -1;
    } else {
        data = BIT_FIFO_RST;
    ...
    g_atk_ms6050_last_fifo_reset_step = 0;
    return 0;
}
...
int mpu_configure_fifo(unsigned char sensors)
{
    ...
    if (st.chip_cfg.dmp_on)
        ...
        g_atk_ms6050_last_fifo_step = 2;
        if (!(st.chip_cfg.sensors))
            return -1;
        prev = st.chip_cfg.fifo_enable;
        st.chip_cfg.fifo_enable = sensors & st.chip_cfg.sensors;
        g_atk_ms6050_last_fifo_step = 3;
        ...
        if (sensors || st.chip_cfg.lp_accel_mode)
            ...
            set_int_enable(0);
        g_atk_ms6050_last_fifo_step = 4;
        if (sensors) {
            g_atk_ms6050_last_fifo_step = 5;
            if (mpu_reset_fifo()) {
...
static float atk_ms6050_clamp_unit(float value)
{
    if (value > 1.0f)
    {
        return 1.0f;
...
uint8_t atk_ms6050_dmp_init(void)
{
    ...
    g_atk_ms6050_last_init_step = 2;
    if (mpu_init(NULL) != 0)
    {
        return ATK_MS6050_EDMP;
    }

    g_atk_ms6050_last_init_step = 3;
    if (mpu_set_sensors(INV_XYZ_GYRO | INV_XYZ_ACCEL) != 0)
    {
        return ATK_MS6050_EDMP;
    }

    g_atk_ms6050_last_init_step = 4;
    if (mpu_configure_fifo(INV_XYZ_GYRO | INV_XYZ_ACCEL) != 0)
    {
        return ATK_MS6050_EDMP;
    }

    g_atk_ms6050_last_init_step = 5;
    if (mpu_set_sample_rate(DEFAULT_MPU_HZ) != 0)
    {
        return ATK_MS6050_EDMP;
    }

    g_atk_ms6050_last_init_step = 6;
    if (dmp_load_motion_driver_firmware() != 0)
    {
        return ATK_MS6050_EDMP;
    }

    g_atk_ms6050_last_init_step = 7;
    if (dmp_set_orientation(inv_orientation_matrix_to_scalar(gyro_orientation)) != 0)
    {
        return ATK_MS6050_EDMP;
    }

    g_atk_ms6050_last_init_step = 8;
    ...
    g_atk_ms6050_last_init_step = 9;
    if (dmp_set_fifo_rate(DEFAULT_MPU_HZ) != 0)
    {
        return ATK_MS6050_EDMP;
    }

    g_atk_ms6050_last_init_step = 10;
    if (mpu_set_int_level(1) != 0)
    {
        return ATK_MS6050_EDMP;
    }

    g_atk_ms6050_last_init_step = 11;
    if (mpu_set_int_latched(0) != 0)
    {
        return ATK_MS6050_EDMP;
    }

    g_atk_ms6050_last_init_step = 12;
    if (mpu_set_dmp_state(1) != 0)
    {
        return ATK_MS6050_EDMP;
    }

    g_atk_ms6050_last_init_step = 13;
    if (atk_ms6050_run_self_test() != 0)
    {
        return ATK_MS6050_ESELFTEST;
    }

    g_atk_ms6050_last_init_step = 0;
...
int atk_ms6050_get_clock_ms(unsigned long *count)
{
    ...
    {
        count[0] = ((unsigned long)xTaskGetTickCount() * 1000UL) / configTICK_RATE_HZ;
        return 0;
    }
#endif
...
uint8_t atk_ms6050_get_last_init_step(void)
{
    return g_atk_ms6050_last_init_step;
}

uint8_t atk_ms6050_get_last_mpu_init_substep(void)
{
    return g_atk_ms6050_last_mpu_init_substep;
}

uint8_t atk_ms6050_get_last_mpu6500_rev(void)
{
    return g_atk_ms6050_last_mpu6500_rev;
}

uint8_t atk_ms6050_get_last_fifo_step(void)
{
    return g_atk_ms6050_last_fifo_step;
}

uint8_t atk_ms6050_get_last_fifo_reset_step(void)
{
    return g_atk_ms6050_last_fifo_reset_step;
}
...
uint8_t atk_ms6050_dmp_get_data(float *pitch, float *roll, float *yaw)
{
    ...
    if (sensors & INV_WXYZ_QUAT)
    {
         ...
         */
        sin_pitch = atk_ms6050_clamp_unit((-2.0f * q1 * q3) + (2.0f * q0 * q2));
        *pitch = asinf(sin_pitch) * 57.2957795f;
